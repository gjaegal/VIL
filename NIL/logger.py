...
class Logger:
    ...
    def save_video(self, video, fps):
        """Save video to the log directory."""
        video_path = os.path.join(self._log_dir, "video.mp4")
        video = video[0]
        video = np.transpose(video, [0, 2, 3, 1])  # [N, T, C, H, W] -> [N, T, H, W, C]
    ...
    def log_trajs_as_videos(
        self, trajs, step, max_videos_to_save=2, fps=24, video_title="video"
    ):
        ...
        # save videos to "result.mp4" file
        self.save_video(videos, fps=fps)
